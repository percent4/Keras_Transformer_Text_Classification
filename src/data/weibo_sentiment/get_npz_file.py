...
labels = ['anger', 'disgust', 'fear', 'happiness', 'like', 'sadness', 'surprise']
label_dict = dict(zip(labels, range(len(labels))))

# 获取字符集并写入json文件中
char_set = set()
train_df = pd.read_csv('weibo_sentiment_train.csv').fillna('')
test_df = pd.read_csv('weibo_sentiment_test.csv').fillna('')
